...
def path(curr : Graph.Node, dest : Graph.Node, marks, small_twice = False, p = []):
    ...
    if not curr.is_big_cave():
        if not small_twice:
            small_twice = marks[curr.name] == 1
        marks[curr.name] = marks.get(curr.name, 0) + 1
    ...
    for neig in curr.neighbours:
        ...
        if small_twice:
            ...
            if marks[neig.name] < 1:
                total += path(neig, dest, marks.copy(), small_twice, p.copy())

        else:

            if neig.name != 'start' and marks[neig.name] < 2:
                total += path(neig, dest, marks.copy(), small_twice, p.copy())
    return total
